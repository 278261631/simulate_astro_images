...
struct Options {
    ...
    double size = 6.0;
    double max_mag = 10.0;
    double roll = 0.0;       // positive clockwise
    double psf_sigma = 1.2;  // pixel
    double gain = 2.0;
};
...
void print_usage() {
    std::cout
        << "Usage: render_sky_patch [options] --out <file>\n"
        << "Options:\n"
        << "  --ra <deg>          Center RA in degrees (default: 83.82, M42)\n"
        << "  --dec <deg>         Center Dec in degrees (default: -5.3875, M42)\n"
        << "  --fov <deg>         Field of view in degrees (default: 30.0)\n"
        << "  --out <path>        Output image path (required, BMP format)\n"
        << "  --catalog <path>    HIP catalog CSV path (default: data/hip_catalog.csv)\n"
        << "  --dpi <int>         Output DPI (default: 200)\n"
        << "  --size <float>      Output size in inches (default: 6.0)\n"
        << "  --max-mag <float>   Max magnitude to render (default: 10.0)\n"
        << "  --roll <deg>        Camera roll, positive clockwise (default: 0.0)\n"
        << "  --psf-sigma <px>    PSF Gaussian sigma in px (default: 1.2)\n"
        << "  --gain <float>      Tone mapping gain (default: 2.0)\n";
}
...
Options parse_args(int argc, char* argv[]) {
    ...
    if (opt.dpi <= 0) {
        throw std::runtime_error("--dpi must be > 0");
    }
    if (opt.size <= 0.0) {
        throw std::runtime_error("--size must be > 0");
    }
    if (opt.psf_sigma <= 0.0) {
        throw std::runtime_error("--psf-sigma must be > 0");
    }
    if (opt.gain <= 0.0) {
        throw std::runtime_error("--gain must be > 0");
    }
...
fs::path build_output_path(const fs::path& base, double ra, double dec, double fov, double roll) {
    ...
    const std::string suffix = "_ra" + format_value(ra) + "_dec" + format_value(dec) +
                               "_fov" + format_value(fov) + "_roll" + format_value(roll);
    return base.parent_path() / (stem + suffix + ext);
}

Catalog load_catalog(const fs::path& input_path, double max_mag) {
    fs::path path = input_path;
    if (!fs::exists(path)) {
        fs::path alt = fs::path("..") / input_path;
        if (fs::exists(alt)) {
            path = alt;
        } else {
            throw std::runtime_error("Catalog not found: " + input_path.string());
        }
    }
    ...
    Catalog c;
    c.ra.reserve(120000);
    c.dec.reserve(120000);
    c.mag.reserve(120000);
...
std::vector<double> angular_distance_deg(
    ...
    for (size_t i = 0; i < ra_deg.size(); ++i) {
        ...
        const double cos_d = std::clamp(
            std::sin(dec) * sin_dec0 + std::cos(dec) * cos_dec0 * std::cos(ra - ra0),
            -1.0,
            1.0
        );
...
Projected project_and_filter(const Catalog& c, const Options& opt) {
    ...
    Projected p;
    p.x.reserve(c.ra.size());
    p.y.reserve(c.ra.size());
    p.mag.reserve(c.ra.size());
    ...
    const double theta = -deg2rad(opt.roll);  // positive is clockwise
    ...
    for (size_t i = 0; i < c.ra.size(); ++i) {
        ...
        if (std::abs(x) > half_extent || std::abs(y) > half_extent) continue;
...
std::vector<float> gaussian_kernel_1d(double sigma) {
    ...
    for (float& x : k) x = static_cast<float>(x / sum);
    return k;
}

std::vector<float> convolve_x(
